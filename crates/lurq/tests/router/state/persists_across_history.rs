  ...
  fn create(ctx: &mut Ctx) -> Self {
    let router = ctx.router(
      Routes::new()
        ...
        .route("/other", |_ctx| Text::new("other").into()),
    );
...
#[test]
fn state_is_restored_when_returning_to_an_entry_via_back() {
...
#[test]
fn replace_with_state_attaches_state_to_the_current_entry() {
  let (mut tree, router) = setup();

  router.replace_with_state(
    "/detail",
    NavState {
      from: "replaced".to_owned(),
    },
  );
  run_pass(&mut tree);
