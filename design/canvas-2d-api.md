...
- **Execution (D9):** use the bounded CPU raster option from section 13.3. A shared tiny-skia surface owns pixels, state, paths, clips, and revisions. WGPU and DX12 both compose its immutable per-revision image through the existing image pipeline. Native GPU path execution remains a performance follow-up; there is no pending drawing-command queue.
- **Detached writes (D3):** rasterize synchronously into the original retained CPU bitmap. Removal clears the ref binding and presentation waker. Old handles never switch to a replacement surface, and their pixels survive window destruction. No hidden command backlog accumulates.
- **Sizing (D2):** compatible rerenders preserve content; logical content resize resets pixels/state/path; scale-only resize resamples pixels and clips. The content rectangle excludes padding. Metrics observers receive initial metrics immediately and subsequent committed changes outside internal locks. Their subscription token must be retained.
- **Threading (D6):** handles are `Send + Sync`; individual operations serialize. Multi-call sequences require application coordination. Pixel writes wake the window through the existing waker and coalesce until painting; custom hosts can install `Tree::set_canvas_waker`.
- **Images/features:** `canvas` implies raw `raster` transport plus tiny-skia, without enabling image codecs or SVG parsing. Raw immutable RGBA `draw_image` is available with `canvas` alone. Add `image` for decoding. Animated/streaming/native/video sources are explicitly unsupported.
- **Text:** cosmic-text and Swash use a shared snapshot of the app font database/aliases. `CanvasFont` snapshots the theme default at first readiness. Text drawing and measurement are fallible; plain single-line shaping, typed alignment/baseline, and ink/font metrics are included.
- **Snapshots:** `CanvasHandle::snapshot()` is implemented now because the authoritative pixels already reside on the CPU. It synchronously copies straight-alpha sRGB RGBA8 bytes and identifies the content revision. It is not a window screenshot and needs no GPU readback.
- **Limits/diagnostics:** backing dimension/pixel, save stack, path segment, distinct clip memory, and text work limits are enforced. DevTools identifies a Canvas node and shows sizes, backing scale, surface/content revision, color-byte estimate, and last error. There are no queued-command diagnostics because there is no command queue. `CanvasStatus` reports canvas-side errors; GPU presentation failures continue to use renderer diagnostics/retry.
- **Presentation:** background and borders remain node styling. Pixels integrate with clipping, transforms, radius, and inherited opacity using image placement. Internal bitmap blending uses sRGB channel values; final window image composition follows the existing linear-light image pipeline.

The initial drawing subset includes solid styles, source-over alpha, save/restore, transforms, rectangles, paths and arcs, curves, fill rules, dashes, clipping, geometry hit tests, text, immutable image crops, and snapshots. Full browser conformance, gradients, patterns, shadows, filters, extra blend modes, `stroke_text`, pixel upload, native image snapshots, and canvas-to-canvas copies are deferred as described in the guide.

The main cost is synchronous rasterization plus conversion/upload of a full changed bitmap. Native GPU execution, dirty-region uploads, and workload benchmarks remain future optimization work; this change makes no frame-rate promise.

Verification includes headless lifecycle/drawing tests and hidden Win32 GPU captures on both backends, including transparent erasure and repaint without layout. The runnable example is `crates/lurq/examples/canvas.rs`; the reproducible capture harness is `canvas_capture_check.rs`.

Validation on Windows: the complete all-features library/integration run passed 1,192 tests. Subsequent hardening passed all 167 library tests and the final 28 canvas tests. Workspace all-features/all-targets compilation, the default build, canvas with resources/Markdown/SVG without image codecs, all-feature examples, and the docs build passed. WGPU and DX12 captures verified pixel output, incremental upload without layout, rounded presentation, and hiding/showing through opacity. Existing unused-code/test-mutability and docs-tool deprecation warnings remain.
